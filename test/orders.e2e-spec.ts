...
test('paginate orders', async ({ page }) => {
  ...
  page.getByRole('button', { name: 'Próxima página' }).click();
  ...
  page.getByRole('button', { name: 'Última página' }).click();
  await page.waitForTimeout(5000);
  ...
  page.getByRole('button', { name: 'Página anterior' }).click();
  await page.waitForTimeout(5000);
  ...
  page.getByRole('button', { name: 'Primeira página' }).click();
  await page.waitForTimeout(5000);
  expect(page.getByRole('cell', { name: 'Customer 1', exact: true })).toBeVisible()
  expect(page.getByRole('cell', { name: 'Customer 10', exact: true })).toBeVisible()
});
